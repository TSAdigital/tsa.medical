...
class CounterpartiesController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                $action_history->ActionHistory('fas fa-plus bg-green', 'добавил(а) контрагента', 'counterparties/view', $model->getId(), $model->name);
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->status === 10) {
            if ($model->load(Yii::$app->request->post())) {
                if ($model->save()) {
                    $action_history->ActionHistory('fas fa-pencil-alt bg-blue', 'отредактировал(а) контрагента', 'counterparties/view', $model->getId(), $model->name);
                    Yii::$app->session->setFlash('success', [
                        'options' => [
    ...
    public function actionUpdateAddress($id, $address)
    {
        ...
        if ($model->status === 10) {
            if ($model->load(Yii::$app->request->post())) {
                if ($model->save()) {
                    $addressHistory = $model->getAddressName();
                    $addressHistory = 'отредактировал(а) ' . Html::a($addressHistory, ['counterparties/view-address', 'id' => $counterparty->id, 'address' => $model->id]) . ' адрес у контрагента';
    ...
    public function actionActiveAddress($id, $address)
    {
        ...
        if ($address->status == 10) {
            ...
            $addressHistory = 'активировал(а) ' . Html::a($addressHistory, ['counterparties/view-address', 'id' => $model->id, 'address' => $address->id]) . ' адрес у контрагента';
    ...
    public function actionBlockedAddress($id, $address)
    {
        ...
        if ($address->status == 9) {
            $addressHistory = $address->getAddressName();
            $addressHistory = 'аннулировал(а) ' . Html::a($addressHistory, ['counterparties/view-address', 'id' => $model->id, 'address' => $address->id]) . ' адрес у контрагента';
    ...
    public function actionBlocked($id)
    {
        ...
        if ($model->status == 9) {
            $action_history->ActionHistory('fas fa-times bg-red', 'аннулировал(а) контрагента', 'counterparties/view', $model->getId(), $model->name);
            Yii::$app->session->setFlash('success', [
                'options' => [
                    'title' => 'Запись аннулирована',
                    'toast' => true,
    ...
    public function actionActive($id)
    {
        ...
        if ($model->status == 10) {
            $action_history->ActionHistory('fas fa-check bg-info', 'активировал(а) контрагента', 'counterparties/view', $model->getId(), $model->name);
            Yii::$app->session->setFlash('success', [
                'options' => [
                    'title' => 'Запись активирована',
                    'toast' => true,
