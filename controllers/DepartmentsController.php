...
class DepartmentsController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        $old = $model->name;
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $action_history->ActionHistory('fas fa-building bg-blue', 'отредактировал(а) подразделение', 'departments/view', $model->getId(), $old . ' <i class="fas fa-code" style="font-size: 13px"></i> ' .$model->name);
            return $this->redirect(['view', 'id' => $model->id]);
        }
